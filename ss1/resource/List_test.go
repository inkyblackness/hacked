...
func someResource(key byte) *resource.Resource {
...
func TestListWith(t *testing.T) {
	t.Run("nil list", func(t *testing.T) {
		...
		assert.Equal(t, resource.List([]*resource.Resource{someResource(0x01)}), result)
	})

	t.Run("append", func(t *testing.T) {
		base := resource.List([]*resource.Resource{someResource(0x01)})
		result := base.With(someResource(0x03))
		assert.Equal(t, resource.List([]*resource.Resource{someResource(0x01), someResource(0x03)}), result)
	})

	t.Run("immutable source", func(t *testing.T) {
		raw := make([]*resource.Resource, 1, 2)
		raw[0] = someResource(0x02)
		base := resource.List(raw)
		result := base.With(someResource(0x04))
		assert.Equal(t, 2, len(result))
		assert.Equal(t, resource.List([]*resource.Resource{someResource(0x02)}), base)
	})
}
...
func TestListJoined(t *testing.T) {
	...
	t.Run("two lists", func(t *testing.T) {
		...
		assert.Equal(t, resource.List([]*resource.Resource{someResource(0x01), someResource(0x02), someResource(0x03), someResource(0x04)}), result)
	})
